...
estim.pars = GenSA(par = params, fn = model, lower = par_lo, upper= par_hi, control = list(verbose =TRUE, smooth=FALSE, max.time = 150000, temperature = 7000, nb.stop.improvement=1000, maxit= 100,  trace.fn = paste("../estimated_params/traceMat_", initForFit, option, "2.trMat", sep="")), dat = datSel)
...
write.table(estim.pars$par,file=paste("../estimated_params/GenSA_", initForFit, option, "2.txt", sep=""), quote=FALSE, col.names=FALSE)
save(estim.pars, file=paste("../estimated_params/GenSA_", initForFit, option, "2.RData", sep=""))
