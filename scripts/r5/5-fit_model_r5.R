...
#estim.pars = GenSA(par = params, fn = model, lower = par_lo, upper= par_hi, control = list(verbose =TRUE, smooth=FALSE, temperature = 7000, nb.stop.improvement=1000, maxit= 10000, trace.fn = paste("../../estimated_params/r5/traceMat_", name,".trMat", sep="")), dat = datSel, step=step)
estim.pars = GenSA(par = params, fn = model, lower = par_lo, upper= par_hi, control = list(verbose =TRUE, smooth=FALSE, temperature = 7000, max.time= 80000, trace.fn = paste("../../estimated_params/r5/traceMat_", name,".trMat", sep="")), dat = datSel, step=step)
...
write.table(estim.pars$par,file=paste("../../estimated_params/r5/GenSA_", name, ".txt", sep=""), quote=FALSE, col.names=FALSE)
save(estim.pars, par_lo, par_hi, file=paste("../../estimated_params/r5/GenSA_", name, ".RData", sep=""))
