...
query_SDMClimate_grid  <- "SELECT ST_X(geom) as lon, ST_Y(geom) as lat, val, biovar FROM (
    ...
    AND biovar IN ('annual_mean_temp', 'pp_seasonality', 'pp_warmest_quarter', 'mean_diurnal_range','tot_annual_pp', 'mean_temp_wettest_quarter')) AS rast_noram,
...
res_SDMClimate_grid$biovar <- factor(res_SDMClimate_grid$biovar,labels=c("Annual mean temperature (°C)",
...
map_var <- function(dat,var,pal,n){
    ...
    ggplot(datvar) +
    geom_polygon(data = regions_usa, aes(x = long, y = lat, group = group),fill="grey80",colour="grey50",size=0.3) +
    geom_polygon(data = regions_can, aes(x = long, y = lat, group = group),fill="grey80",colour="grey50",size=0.3) +
    geom_raster(aes(lon,lat,fill=val, order = rev(val))) +
 ...
 for(i in 1:nlevels(res_SDMClimate_grid$biovar)){
    ...
    ggsave(paste("../figures/var",i,"_1970-2000.pdf",sep=""),height=5,width=7)
 }