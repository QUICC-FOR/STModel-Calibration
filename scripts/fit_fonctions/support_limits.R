support_limits = function (model, par, var, source_data, pdf, par_lo = NULL, par_hi = NULL, 
    delta = 100, slimit = 2) 
{
    if (is.null(par_lo)) {
        par_lo <- par
    ...
    for (i in 1:length(par)) {
        if (!any(names(par_lo) == names(par)[i])) {
            par_lo[[names(par)[i]]] = rep(-.Machine$double.xmax, 
                times = length(par[[i]]))
        }
        if (!any(names(par_hi) == names(par)[i])) {
            par_hi[[names(par)[i]]] = rep(.Machine$double.xmax, 
                times = length(par[[i]]))
        }
    }
    for (i in 1:length(par_lo)) {
        par_lo[[i]] = ifelse(is.infinite(par_lo[[i]]), -.Machine$double.xmax, 
            par_lo[[i]])
    }
    for (i in 1:length(par_hi)) {
        par_hi[[i]] = ifelse(is.infinite(par_hi[[i]]), .Machine$double.xmax, 
            par_hi[[i]])
    }
    lower_limit <- par
    upper_limit <- par
