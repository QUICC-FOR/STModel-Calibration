    ...
    if (fun_args[[i]] == "...") {
      ...
      if(any(used)) {
        if(is.function(par$value[used][[1]])) {
          ...
          # Check this for being one of the neighborhood functions - evaluate 
          # them specially (if the neighlikeli package is loaded)
          if (any(search() == "package:neighlikeli") && 
             (identical(par$value[used][[1]], sumneigh) || 
              identical(par$value[used][[1]], sumneigh_lognormal_disperse))) {            
            results$pre_eval[[length(results$pre_eval)+1]]<-sumneigh_setup(par$value[used][[1]], par, fun_args[[i]], ...)
        ...
        if(any(used)) {
          if (!any(fun_args=="...")) {
            if(is.function(extra_args[used][[1]])) {
              # One of the arguments is a function - recursively analyze it if
              # evaluate nested flag is true; otherwise just pass as an unevaluated
              # function
              if (eval_nested) {
                results$pre_eval[[length(results$pre_eval)+1]]<-analyze_function(extra_args[used][[1]], par, fun_args[[i]], dataset, ...)
                results$call[[fun_args[[i]]]]<-parse(text=paste("eval_results$", fun_args[[i]],sep=""))[[1]]
              } 
              else {
